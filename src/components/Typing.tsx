...
export default function Typing(props: typingProps) {
  ...
  const isThePressedKeyCorrect = (key: string) =>
    key === typingPara[getCurrentWord()][getCurrentLetter()]
  const isLastLetter = () =>
    getCurrentLetter() === typingPara[getCurrentWord()].length - 1
  const nextWord = (currentWord: number) => () =>
    changePos({word: currentWord + 1, letter: 0})
  const nextLetter = (currentLetter: number) => () =>
    changePos({word: getCurrentWord(), letter: currentLetter + 1})
  const previousWord = (currentWord: number) => ({
    word: currentWord - 1,
    letter: typingPara[currentWord - 1].length - 1,
  })
  const previousLetter = (currentLetter: number) => ({
    word: getCurrentWord(),
    letter: currentLetter - 1,
  })
  const reachedEnd = () =>
    getCurrentWord() + 1 === typingPara.length &&
    typingPara[getCurrentWord()].length === getCurrentLetter() + 1
  const isVeryFirstLetter = () =>
    getCurrentWord() === 0 && getCurrentLetter() === 0
  ...
  const notConcernedKey = (e: React.KeyboardEvent) =>
    notRequiredKeys.includes(e.key)
  ...
  const startCalculating = () => {
    return setInterval(() => {
      const result = calculateSpeedAndAccuracy(
        startTime,
        entries.current,
  ...
  const handleTyping = (e: React.KeyboardEvent<HTMLElement>): void => {
    ...
    if (getCurrentWord() === 0 && getCurrentLetter() === 0) {
      ...
      const getSpeedAndAccuracy = R.partial(calculateSpeedAndAccuracy, [
        startTime,
        entries.current + 1,
      ])
      clearInterval(timer)
      console.log('timer cleared from onKeyPress', timer)
      const speedAndAccuracy =
        e.key === typingPara[getCurrentLetter()]
  ...
  const handleBackSpace = (): void => {
    ...
    R.includes(newPosition, correct.current)
      ? changeCorrect(
          R.slice(0, R.indexOf(newPosition, correct.current), correct.current),
        )
        ...
        {typingPara.map((word, wordIndex) => (
          <span key={wordIndex} className="word">
            {[...word].map((letter, letterIndex) => {
              ...
              if (
                wordIndex === getCurrentWord() &&
                letterIndex === getCurrentLetter()
              ) {
                return (
                  <span
                    key={`${wordIndex}-${letterIndex}`}
                    data-testid="typing-current"
                ...
                return (
                  <>
                    {wrongLetter &&
                    getCurrentWord() === wordIndex &&
                    getCurrentLetter() - 1 === letterIndex ? (
                      <span
                        key={`wrong-${wordIndex}- ${letterIndex}`}
...
function useAsyncRef<T>(
  value: T,
): [React.MutableRefObject<T>, (newVal: T) => void] {
